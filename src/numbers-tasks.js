...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const lenghtVectorA = Math.sqrt(x1 * x1 + y1 * y1);
  const lengthVectorB = Math.sqrt(x2 * x2 + y2 * y2);

  const cosinusValue = dotProduct / (lenghtVectorA * lengthVectorB);
  return Math.acos(cosinusValue);
}
...
function toNumber(value, def) {
  if (!!value === false || Number.isNaN(Number(value))) return def;
...
function getSumOfDigits(num) {
  ...
  return arr.reduce((acc, currentN) => acc + +currentN, 0);
}
 ...
 */
function isNumber(n) {
  return typeof n === 'number' && n !== Infinity && Number.isFinite(n);
}
